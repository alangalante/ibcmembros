...
async function run() {
  requireProductionConfirmation(process.argv.slice(2));
  ...
  for (const member of preview.members) {
    const auth = await adminAuth.createUser({ email: `${member.username}@ibcmembros.internal`, password: member.initialPassword, displayName: member.name, disabled: false });
