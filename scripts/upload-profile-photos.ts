...
async function run() {
  ...
  for (const photo of manifest) {
    ...
    if (user.get("photoPublicId") === publicId && user.get("photoUrl")) { completed++; continue; }
    ...
    form.set("api_key", apiKey); form.set("timestamp", String(timestamp)); form.set("public_id", publicId); form.set("overwrite", "true"); form.set("signature", signature);
