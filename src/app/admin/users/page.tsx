...
export default function AdminUsersPage() {
  ...
  const filteredUsers = offline.users.filter((item) => {
    const matchesSearch =
      ...
      item.phoneE164.includes(search);
  ...
  async function openEditModal(uid: string) {
    ...
    try {
      ...
      /* Ignora falha de busca privada se offline */
    } finally {
    ...
    <div className="min-h-dvh bg-slate-50 text-slate-900 pb-20">
      ...
      <main className="mx-auto max-w-lg px-4 pt-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Membros & Visitantes</h1>
            <p className="text-xs text-slate-500">{filteredUsers.length} registros cadastrados</p>
          </div>
          <button
        ...
        <div className="mt-4 space-y-2">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm outline-none focus:border-emerald-700"
          />
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl max-h-[90dvh] overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-slate-700">Telefone (E.164) *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="border-t border-slate-100 pt-3">
                <p className="text-xs font-bold text-slate-500 mb-2">Conta de Acesso (Opcional)</p>
