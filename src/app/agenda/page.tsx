...
export default function AgendaPage() {
  ...
  const renderList = (items: typeof monthItems) => <div className="mt-5 space-y-3">{items.map((item) => { const passed = item.date < today; return <article key={item.id} className={`flex items-center justify-between gap-3 rounded-2xl border p-4 shadow-xs ${passed ? "border-slate-200 bg-slate-100 opacity-50 grayscale" : "border-slate-100 bg-white"}`}><button onClick={() => item.kind === "birthday" && setSelectedUid(item.userId)} className="text-left"><div className="flex items-center gap-2"><p className={`text-[11px] font-bold capitalize ${passed ? "text-slate-500" : "text-emerald-800"}`}>{formatAgendaDate(item.date)}</p>{passed && <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[9px] font-bold uppercase text-slate-600">Já passou</span>}</div><div className="mt-1 flex flex-wrap items-center gap-2"><h2 className="text-sm font-bold">{item.kind === "birthday" ? "🎂" : "📅"} {item.title}</h2>{item.kind === "agenda" && item.pdfUrl && <span className="rounded-full bg-rose-50 px-2 py-1 text-[9px] font-bold text-rose-700" aria-label="Esta agenda possui PDF anexado">📎 PDF</span>}</div></button>{item.kind === "agenda" && <div className="flex gap-2">{item.pdfUrl && <a href={item.pdfUrl} target="_blank" rel="noreferrer" className={`text-xs font-bold ${passed ? "text-slate-600" : "text-rose-700"}`}>Ver PDF</a>}<Link href={`/events/${item.id}`} className={`text-xs font-bold ${passed ? "text-slate-600" : "text-emerald-800"}`}>Detalhes →</Link></div>}</article>; })}{!items.length && <p className="rounded-2xl bg-white p-8 text-center text-sm text-slate-500">Nenhum item neste período.</p>}</div>;
          ...
          {days.map((date) => {
            ...
            return <section key={date} className={`rounded-2xl border bg-white p-4 shadow-xs ${passed ? "opacity-45 grayscale" : date === today ? "border-emerald-500 ring-2 ring-emerald-100" : "border-slate-100"}`}>
              ...
              <div className="mt-3 space-y-2">
                ...
                {agendas.map((event) => <Link key={event.id} href={`/events/${event.id}`} className="block rounded-xl border border-emerald-100 bg-emerald-50 p-3"><span className="flex items-center justify-between gap-2 text-xs font-bold text-emerald-950"><span>📅 {event.title}</span>{event.pdfUrl && <span className="shrink-0 rounded-full bg-rose-100 px-2 py-1 text-[9px] text-rose-700" aria-label="Esta agenda possui PDF anexado">📎 PDF</span>}</span><span className="text-[10px] text-emerald-800">{event.scope === "global" ? "Agenda global" : "Agenda do seu grupo"}</span></Link>)}
